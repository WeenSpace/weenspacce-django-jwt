...
from django_jwt.refresh_token.signals import (
    refresh_token_revoked,
    refresh_token_rotated,
)
from django_jwt.settings import jwt_settings
from django_jwt.shortcuts import create_refresh_token, get_refresh_token
from django_jwt.signals import token_issued
...
class RefreshTokenMutationMixin:
    @override_jwt_settings(JWT_REFRESH_TOKEN_LONG_RUNNING=True)
    def setUp(self):
        self.Mutation = type(
            "jwt",
            (graphene.ObjectType,),
...
class RefreshMixin(RefreshTokenMutationMixin, RefreshTokenMixin):
    ...
    @override_jwt_settings(JWT_REFRESH_TOKEN_REUSE=True)
