...
class JWTMixin:
    ...
    def Field(cls, *args, **kwargs):
        if not jwt_settings.JWT_HIDE_TOKEN_FIELDS:
            ...
            if jwt_settings.JWT_REFRESH_TOKEN_LONG_RUNNING:
...
class CreateJWTMixin(JWTMixin):
    @classmethod
    def __init_subclass_with_meta__(cls, name=None, **options):
        assert getattr(cls, "create", None), (
            f"{name or cls.__name__}.create "
            "method is required in a JWTMutation."
        )
...
class KeepAliveRefreshMixin:
    ...
    def refresh(cls, root, info, token, **kwargs):
        ...
        if not orig_iat:
            raise exceptions.JWTError(_("origIat field is required"))

        if jwt_settings.JWT_REFRESH_EXPIRED_HANDLER(orig_iat, context):
            raise exceptions.JWTError(_("Refresh has expired"))
